...
// startReadyTimeout bounds how long the host's Start RPC waits for the
// plugin's impl.Start to return synchronously. If impl.Start has not
// returned by then, the plugin is treated as long-running (it blocks inside
// its run loop) and the host RPC returns success. The plugin's runCtx is
// still cancelled by Stop, so cleanup remains correct.
const startReadyTimeout = 2 * time.Second
...
//
// impl.Start is invoked in a goroutine so that a long-running plugin that
// uses Start as a run loop does not wedge the host's lifecycle RPC. We wait
// up to startReadyTimeout for synchronous startup errors; if Start hasn't
// returned by then, the plugin is treated as long-running and the host RPC
// returns success. Stop() still cancels runCtx, which the plugin observes.
func (s *pluginGRPCServer) Start(_ context.Context, _ *pluginrpcpb.Empty) (*pluginrpcpb.Empty, error) {
	...
	case <-time.After(startReadyTimeout):
		// Long-running plugin: still inside its run loop. Treat as started.
		// Drain resultCh asynchronously so a late impl.Start error does not
		// disappear silently. We cancel runCtx and tear down the host
		// connection so the subprocess exits, which triggers the host's
		// watchHealth callback (PluginManager.onUnexpectedExit) and surfaces
		// the failure as an instance-level error. log.Println is the only
		// option from inside the plugin process; the line is captured by
		// go-plugin's stderr forwarder and ends up in the host log.
		go func() {
			err := <-resultCh
			if err == nil {
				return
			}
			log.Printf("plugin Start returned error after timeout window: %v", err)
			s.mu.Lock()
			cancel := s.runCancel
			s.runCancel = nil
			s.runCtx = nil
...
// Stop cancels the run context and calls the plugin's Stop method. Zeros
// runCancel/runCtx/hostConn under the lock for symmetry with the late-error
// drainer in Start; this prevents the drainer from racing on a connection
// Stop already closed (grpc.ClientConn.Close is idempotent, but the
// asymmetry was a footgun).
...
func (c *PluginGRPCClient) StartHostAPIBroker(register func(*grpc.Server)) (uint32, func(), error) {
	...
	// stop is safe under concurrent invocation. The select-default-close idiom
	// is racy with two callers (e.g. plugin Stop() and the timeout-fallback
	// Kill() path racing during the 30s shutdown window), so use sync.Once.
