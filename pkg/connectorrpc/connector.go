...
// Start runs the connector's Start in a goroutine. We wait up to
// startReadyTimeout for synchronous errors; if Start hasn't returned by then,
// the connector is treated as long-running (it blocks inside its run loop)
// and the host RPC returns success. Stop still cancels runCtx, which the
// connector observes.
func (s *connectorGRPCServer) Start(_ context.Context, _ *connectorrpcpb.Empty) (*connectorrpcpb.Empty, error) {
	...
	case <-time.After(startReadyTimeout):
		// Long-running connector: still inside its run loop.
	}
	return &connectorrpcpb.Empty{}, nil
}

// startReadyTimeout bounds how long the host's Start RPC waits for the
// connector's impl.Start to return synchronously. See pluginrpc for the
// rationale; connectors share the same long-running pattern.
...
// timeout carried on the wire. The inbound gRPC context is preserved so that
// host-side cancellation (shutdown, plugin Stop, client disconnect) propagates
// to the connector implementation.
func (s *connectorGRPCServer) Invoke(ctx context.Context, req *connectorrpcpb.InvokeRequest) (*connectorrpcpb.InvokeResponse, error) {
	if ctx == nil {
