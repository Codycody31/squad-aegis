...
func RegisterUser(ctx context.Context, database db.Executor, user *models.User) (*models.User, error) {
	...
	sql, args, err := psql.Insert("users").Columns("steam_id", "name", "username", "password", "super_admin").Values(user.SteamId, user.Name, user.Username, user.Password, user.SuperAdmin).ToSql()
	if err != nil {
		return nil, err
	}
