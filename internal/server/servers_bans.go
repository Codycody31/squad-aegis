...
func (s *Server) ServerBansList(c *gin.Context) {
	...
	rows, err := s.Dependencies.DB.QueryContext(c.Request.Context(), `
		SELECT sb.id, sb.server_id, sb.admin_id, u.username, sb.steam_id, sb.reason, sb.duration, sb.rule_id, sr.title as rule_title, sr.display_order as rule_number, sb.ban_list_id, bl.name as ban_list_name, sb.created_at, sb.updated_at
		FROM server_bans sb
	...
	for rows.Next() {
		...
		// Set rule number if present
		if ruleNumber.Valid {
			ruleNumStr := strconv.Itoa(int(ruleNumber.Int32))
			ban.RuleNumber = &ruleNumStr
		}
...
func (s *Server) ServerBansUpdate(c *gin.Context) {
	...
	if ruleNumber.Valid {
		ruleNumStr := strconv.Itoa(int(ruleNumber.Int32))
		currentBan.RuleNumber = &ruleNumStr
	}
	...
	var updatedRuleNumber sql.NullInt32
	...
	// Set rule number if present
	if updatedRuleNumber.Valid {
		ruleNumStr := strconv.Itoa(int(updatedRuleNumber.Int32))
		updatedBan.RuleNumber = &ruleNumStr
	}
