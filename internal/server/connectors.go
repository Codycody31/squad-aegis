...
func (s *Server) ConnectorCreate(c *gin.Context) {
	...
	if err != nil {
		...
		// Include the underlying error in data.error so clients see
		// validation/dependency reasons.
...
func (s *Server) ConnectorPackageUpload(c *gin.Context) {
	...
	// confirm_unsafe is only consulted when the parsed bundle turns out to
	// be unsigned. Signed bundles install without it.
