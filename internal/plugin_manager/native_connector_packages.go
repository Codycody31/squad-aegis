...
// InstallConnectorPackageFromBundleWithUnsafeConfirm gates installation of
// an unsigned sideloaded connector on confirm_unsafe. Signed bundles install
// without it.
...
func (pm *PluginManager) installConnectorBundleWithFlags(ctx context.Context, archive io.ReaderAt, size int64, originalName string, distribution PluginDistribution, confirmUnsafe bool) (*InstalledConnectorPackage, error) {
	...
	// Pin the SHA on the in-memory struct only. The raw manifest bytes from
	// the bundle are preserved for signature verification and persistence;
	// re-marshaling the Go struct drops omitempty fields and breaks the
	// canonical equality check inside verifyManifestSignature.
	...
	if distribution == PluginDistributionSideload && !signatureVerified && !allowUnsafeSideload() {
		if len(parts.SignatureBytes) > 0 && (verification.Payload.KeyID != "" || isRevokedPublicKey(parts.PublicKeyBytes)) {
			return nil, fmt.Errorf("connector signature rejected: %s", formatVerificationFailure(verification.Payload, parts.PublicKeyBytes))
		}
	...
	// Stash the existing library on same-version reinstall so a DB save
	// failure does not delete the only on-disk copy.
	previousFileExists := false
	if _, statErr := os.Stat(runtimePath); statErr == nil {
		previousFileExists = true
	} else if !os.IsNotExist(statErr) {
		return nil, fmt.Errorf("failed to inspect existing connector runtime file: %w", statErr)
	}
	...
	if willRequireRestart {
		...
		if _, statErr := os.Stat(stashPath); statErr == nil {
			return nil, fmt.Errorf("found stale connector runtime stash %s; resolve before reinstalling (likely a previous install was interrupted)", stashPath)
		} else if !os.IsNotExist(statErr) {
			return nil, fmt.Errorf("failed to inspect connector runtime stash: %w", statErr)
		}
		if err := os.Rename(runtimePath, stashPath); err != nil {
			return nil, fmt.Errorf("failed to stash existing connector runtime file: %w", err)
		}
	}
	if err := writeRuntimeLibrary(writePath, parts.LibraryBytes); err != nil {
		if stashPath != "" {
			if rerr := os.Rename(stashPath, runtimePath); rerr != nil {
				log.Warn().Err(rerr).Str("connector_id", manifest.ConnectorID).Str("path", runtimePath).Msg("Failed to restore previous connector runtime file after write failure")
			}
		}
		return nil, err
	}
	...
	defer func() {
		if installCommitted {
			if stashPath != "" {
				if safeStash, pathErr := validateRuntimePathWithinRoot(stashPath, connectorRuntimeDir()); pathErr != nil {
					log.Warn().Err(pathErr).Str("path", stashPath).Msg("Refusing to remove connector runtime stash outside runtime root")
				} else {
		...
		if stashPath != "" {
			if err := os.Rename(stashPath, runtimePath); err != nil {
				log.Warn().Err(err).Str("connector_id", manifest.ConnectorID).Str("path", runtimePath).Msg("Failed to restore previous connector runtime file during rollback")
			}
		}
	}()
