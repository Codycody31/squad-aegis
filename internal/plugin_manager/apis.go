...
type adminAPI struct {
	...
	// pluginID is the canonical plugin identifier; stored as
	// server_admins.managed_by_plugin_id so a recreated instance can claim
	// rows orphaned when its predecessor was deleted.
	pluginID string
}
...
type managedAdminRecord struct {
	...
	ManagedByPluginID       sql.NullString
}

func NewAdminAPI(serverID uuid.UUID, db *sql.DB, rconManager *rcon_manager.RconManager, pluginInstanceID uuid.UUID, pluginID string) AdminAPI {
	return &adminAPI{
		serverID:         serverID,
		db:               db,
		rconManager:      rconManager,
...
func (api *adminAPI) listMatchingAdminRecords(steamIDArg interface{}, eosIDArg interface{}, roleID *uuid.UUID) ([]managedAdminRecord, error) {
	...
	if roleID != nil {
		rows, err = api.db.Query(`
			SELECT id, managed_by_plugin_instance_id, managed_by_plugin_id
			FROM server_admins
			WHERE server_id = $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.ID, &record.ManagedByPluginInstance, &record.ManagedByPluginID); err != nil {
			return nil, err
		}
...
// managedByPluginIDValue returns the SQL value for managed_by_plugin_id.
// Empty pluginID becomes NULL so the orphan-claim path cannot match.
func (api *adminAPI) managedByPluginIDValue() interface{} {
	if api.pluginID == "" {
		return nil
	}
	return api.pluginID
...
// claimOrphanAdminRecords reassigns orphan rows (managed_by_plugin_instance_id IS NULL)
// to the calling instance when managed_by_plugin_id matches. Uses CAS so a
// concurrent claim from a sibling instance wins cleanly. Rows with NULL
// plugin_id (user-typed) are never touched.
func (api *adminAPI) claimOrphanAdminRecords(records []managedAdminRecord) ([]managedAdminRecord, error) {
	if len(records) == 0 || api.pluginID == "" {
		return records, nil
	}

	updated := make([]managedAdminRecord, 0, len(records))
	for _, record := range records {
		instanceValid := record.ManagedByPluginInstance.Valid &&
			strings.TrimSpace(record.ManagedByPluginInstance.String) != ""
		if instanceValid {
			updated = append(updated, record)
			continue
		}
		if !record.ManagedByPluginID.Valid || strings.TrimSpace(record.ManagedByPluginID.String) != api.pluginID {
			updated = append(updated, record)
			continue
		}
		res, err := api.db.Exec(`
			UPDATE server_admins
			SET managed_by_plugin_instance_id = $1
			WHERE id = $2
			  AND managed_by_plugin_instance_id IS NULL
			  AND managed_by_plugin_id = $3
		`, api.pluginInstanceID, record.ID, api.pluginID)
		if err != nil {
			return nil, fmt.Errorf("failed to claim orphan admin record: %w", err)
		}
		affected, _ := res.RowsAffected()
		if affected == 1 {
			record.ManagedByPluginInstance = sql.NullString{String: api.pluginInstanceID.String(), Valid: true}
		}
		// affected == 0: a sibling instance won the claim. Let validation
		// surface the cross-plugin ownership error rather than retrying.
		updated = append(updated, record)
	}
	return updated, nil
}
...
func (api *adminAPI) AddTemporaryAdmin(playerID string, roleName string, notes string, expiresAt *time.Time) error {
	...
	// Reclaim rows orphaned by a previous instance of this plugin.
	existingRecords, err = api.claimOrphanAdminRecords(existingRecords)
	if err != nil {
		return fmt.Errorf("failed to reclaim orphan admin records: %w", err)
	}
	...
	if len(ownedRecordIDs) == 0 {
		// Record both the instance UUID (current run) and the canonical
		// plugin ID (stable across recreation) so a future instance can
		// reclaim this row if the current one is deleted.
		adminID := uuid.New()
		_, err = api.db.Exec(`
			INSERT INTO server_admins (id, server_id, steam_id, eos_id, server_role_id, expires_at, notes, managed_by_plugin_instance_id, managed_by_plugin_id, created_at)
			VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
		`, adminID, api.serverID, steamIDArg, eosIDArg, roleID, expiresAt, notes, api.pluginInstanceID, api.managedByPluginIDValue())
		if err != nil {
...
func (api *adminAPI) RemoveTemporaryAdmin(playerID string, notes string) error {
	...
	// Owned by this instance, or an orphan whose plugin_id matches.
	result, err := api.db.Exec(`
		DELETE FROM server_admins
		WHERE server_id = $1
		  AND (
			managed_by_plugin_instance_id = $2
			OR (managed_by_plugin_instance_id IS NULL
				AND $5::text <> ''
				AND managed_by_plugin_id = $5::text)
		  )
		  AND (
			($3::bigint IS NOT NULL AND steam_id = $3::bigint)
			OR ($4::text IS NOT NULL AND eos_id = $4::text)
		  )
	`, api.serverID, api.pluginInstanceID, steamIDArg, eosIDArg, api.pluginID)
	if err != nil {
...
func (api *adminAPI) RemoveTemporaryAdminRole(playerID string, roleName string, notes string) error {
	...
	// Same orphan-aware match as RemoveTemporaryAdmin.
	result, err := api.db.Exec(`
		...
		WHERE sa.server_role_id = sr.id
			AND sa.server_id = $1
			AND (
				sa.managed_by_plugin_instance_id = $2
				OR (sa.managed_by_plugin_instance_id IS NULL
					AND $6::text <> ''
					AND sa.managed_by_plugin_id = $6::text)
			)
			AND (
			...
			AND sr.server_id = $1
			AND sr.name = $5
	`, api.serverID, api.pluginInstanceID, steamIDArg, eosIDArg, roleName, api.pluginID)
	if err != nil {
...
func (api *adminAPI) ListTemporaryAdmins() ([]*TemporaryAdminInfo, error) {
	...
	// Includes inherited orphans (instance NULL, plugin_id matches).
	rows, err := api.db.Query(`
		...
		WHERE sa.server_id = $1
		  AND (
			...
			OR (sa.managed_by_plugin_instance_id IS NULL
				AND $3::text <> ''
				AND sa.managed_by_plugin_id = $3::text)
		  )
		ORDER BY sa.created_at DESC
	`, api.serverID, api.pluginInstanceID, api.pluginID)
	if err != nil {
...
// recordPluginLog publishes a live log event and (when configured) persists
// to ClickHouse. The event publish runs unconditionally so deployments
// without ClickHouse still get live UI/SSE streams.
func (api *logAPI) recordPluginLog(level, message string, errorMsg *string, fields map[string]interface{}) {
	timestamp := time.Now().UTC()

	// Publish to event manager first so live streams work without ClickHouse.
