...
// nativePluginVerifiedLoader fetches the static definition of a native plugin
// package. Production: spawn a go-plugin subprocess (gRPC + AutoMTLS), peek
// the definition, kill the subprocess. The returned CreateInstance factory
// spawns a fresh subprocess per instance, isolating crashes to the plugin.
// Tests override the hook to inject canned definitions.
...
// ErrUnsignedBundleNeedsConfirm signals that an unsigned/unverified sideload
// requires an explicit operator opt-in. The HTTP handler maps this to a 400
// asking the caller to retry with ?confirm_unsafe=true.
...
// InstallPluginPackageFromBundleWithUnsafeConfirm gates installation on a
// confirm_unsafe flag when the parsed bundle is unsigned. Signed bundles
...
func (pm *PluginManager) installPluginBundleWithFlags(ctx context.Context, archive io.ReaderAt, size int64, originalName string, distribution PluginDistribution, confirmUnsafe bool) (*InstalledPluginPackage, error) {
	...
	// Pin SHA256 on the in-memory struct so downstream load checks can read
	// the expected hash from manifest alone. We do not re-marshal: raw bytes
	// must be preserved for signature verification. Capabilities are part of
	// the match key because validatePluginManifest dedups on them, so two
	// targets can share LibraryPath but differ in capabilities.
	for index := range manifest.Targets {
		if manifest.Targets[index].LibraryPath == selectedTarget.LibraryPath &&
			...
			stringSlicesEqual(manifest.Targets[index].RequiredCapabilities, selectedTarget.RequiredCapabilities) {
			manifest.Targets[index].SHA256 = libraryHash
		}
	}
	selectedTarget.SHA256 = libraryHash
	manifestBytes := parts.ManifestBytes
	...
	if distribution == PluginDistributionSideload && !signatureVerified && !allowUnsafeSideload() {
		if len(parts.SignatureBytes) > 0 && (verification.Payload.KeyID != "" || isRevokedPublicKey(parts.PublicKeyBytes)) {
			return nil, fmt.Errorf("plugin signature rejected: %s", formatVerificationFailure(verification.Payload, parts.PublicKeyBytes))
		}
		return nil, fmt.Errorf("unsigned sideloads are disabled")
	}

	// In unsafe-sideload mode, signed bundles install freely; only unsigned
	// or unverified ones need confirm_unsafe=true.
	...
	// If live instances are using the existing runtime file, write to a
	// .pending side file. promotePendingRuntimeIfPresent atomically swaps it
	// in on next host restart, so a pre-restart lifecycle restart never
	// observes mixed bytes.
	willRequireRestart, restartErr := pm.hasPluginInstances(ctx, manifest.PluginID)
	if restartErr != nil {
		return nil, restartErr
	}
	// On a no-instance reinstall with different bytes, stash the existing
	// library so a DB save failure does not leave the prior row pointing at
	// a deleted file. The deferred cleanup restores the stash on rollback.
	previousFileExists := false
	if _, statErr := os.Stat(runtimePath); statErr == nil {
		previousFileExists = true
	} else if !os.IsNotExist(statErr) {
		return nil, fmt.Errorf("failed to inspect existing plugin runtime file: %w", statErr)
	}
	...
	if willRequireRestart {
		...
		// A pre-existing .prev means a previous install was interrupted
		// before commit or rollback. Refuse rather than overwrite the only
		// surviving copy; the loader's startup sweep will reconcile it.
		stashPath = runtimePath + ".prev"
		if _, statErr := os.Stat(stashPath); statErr == nil {
			return nil, fmt.Errorf("found stale plugin runtime stash %s; resolve before reinstalling (likely a previous install was interrupted)", stashPath)
		} else if !os.IsNotExist(statErr) {
			return nil, fmt.Errorf("failed to inspect plugin runtime stash: %w", statErr)
		}
		if err := os.Rename(runtimePath, stashPath); err != nil {
			return nil, fmt.Errorf("failed to stash existing plugin runtime file: %w", err)
		}
	}
	if err := writeRuntimeLibrary(writePath, parts.LibraryBytes); err != nil {
		// Restore the stashed previous library before returning.
		if stashPath != "" {
			if rerr := os.Rename(stashPath, runtimePath); rerr != nil {
				log.Warn().Err(rerr).Str("plugin_id", manifest.PluginID).Str("path", runtimePath).Msg("Failed to restore previous plugin runtime file after write failure")
			}
		}
		return nil, err
	}
	// On rollback: remove the new bytes and restore the stashed library.
	// On commit: discard the now-superseded stash.
	installCommitted := false
	defer func() {
		if installCommitted {
			if stashPath != "" {
				if safeStash, pathErr := validateRuntimePathWithinRoot(stashPath, pluginRuntimeDir()); pathErr != nil {
					log.Warn().Err(pathErr).Str("path", stashPath).Msg("Refusing to remove plugin runtime stash outside runtime root")
				} else {
					removeRuntimeFile(safeStash)
				}
			}
			return
		}
		...
		if stashPath != "" {
			if err := os.Rename(stashPath, runtimePath); err != nil {
				log.Warn().Err(err).Str("plugin_id", manifest.PluginID).Str("path", runtimePath).Msg("Failed to restore previous plugin runtime file during rollback")
			}
		}
	}()
	...
	if requiresRestart {
		// .pending was written above; live instances keep using runtimePath
		// until restart promotes the new bytes.
...
func (pm *PluginManager) loadNativePluginPackage(pkg *InstalledPluginPackage) error {
	...
	// Rebuild the target snapshot for the peek/merge step from the
	// package's compatibility fields; the full targets array is not stored
	// on InstalledPluginPackage.
	...
	// peek already enforced ID match, so we only overlay install-time state
