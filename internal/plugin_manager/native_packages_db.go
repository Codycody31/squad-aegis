...
func (pm *PluginManager) loadInstalledPluginPackages() error {
	...
	for rows.Next() {
		...
		// JSONB normalization makes this not byte-equal to the bundle's
		// manifest.json. Signature verification reads signed_manifest_json.
		...
		if mustQuarantine {
			if pkg.InstallState == PluginInstallStateReady || pkg.InstallState == PluginInstallStatePendingRestart {
				if previouslyTrusted {
					reason := formatVerificationFailure(verification.Payload, pkg.ManifestPublicKey)
					...
					// Quarantine without removing the runtime file: operators
					// re-upload to recover, and we don't want to delete the
					// .so on every transient trust-state mismatch.
	...
	for _, pkg := range loadedPackages {
		...
		if pkg.InstallState == PluginInstallStatePendingRestart {
			// Promote the .pending side-file written during a same-version
			// upgrade. Atomic rename keeps host crashes from leaving mixed bytes.
			if pkg.RuntimePath != "" {
		...
		// Reconcile orphan .prev stashes from interrupted reinstalls.
		// runtimePath missing: restore the stash. runtimePath present: the
		// stash is a stale leftover, remove it.
		if pkg.RuntimePath != "" {
			prevPath := pkg.RuntimePath + ".prev"
			if _, statErr := os.Stat(prevPath); statErr == nil {
				safePrev, prevPathErr := validateRuntimePathWithinRoot(prevPath, pluginRuntimeDir())
				if prevPathErr != nil {
					log.Warn().Err(prevPathErr).Str("plugin_id", pkg.PluginID).Str("path", prevPath).Msg("Refusing to reconcile native plugin .prev stash outside runtime root")
				} else if _, runtimeStatErr := os.Stat(pkg.RuntimePath); os.IsNotExist(runtimeStatErr) {
					if safeRuntime, runtimePathErr := validateRuntimePathWithinRoot(pkg.RuntimePath, pluginRuntimeDir()); runtimePathErr == nil {
						if renameErr := os.Rename(safePrev, safeRuntime); renameErr == nil {
							log.Info().Str("plugin_id", pkg.PluginID).Str("path", safeRuntime).Msg("Restored native plugin runtime file from .prev stash after interrupted install")
						} else {
							log.Warn().Err(renameErr).Str("plugin_id", pkg.PluginID).Str("from", safePrev).Str("to", safeRuntime).Msg("Failed to restore native plugin runtime file from .prev stash")
						}
					}
				} else {
					if removeErr := os.Remove(safePrev); removeErr != nil {
						log.Warn().Err(removeErr).Str("plugin_id", pkg.PluginID).Str("path", safePrev).Msg("Failed to remove stale native plugin .prev stash")
					} else {
						log.Info().Str("plugin_id", pkg.PluginID).Str("path", safePrev).Msg("Removed stale native plugin .prev stash from previous successful install")
					}
				}
			}
		}
