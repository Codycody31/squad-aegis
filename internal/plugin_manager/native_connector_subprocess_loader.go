		...
		case <-ticker.C:
			...
			if handle.client.Exited() {
				...
				if !intentional {
					...
					// Stop() already wrote ConnectorStatusStopped.
...
//
// The handle field is not cleared until after handle.Kill() completes, so a
// concurrent Kill() (the host-side SIGKILL fallback when stopConnectorInstance
// times out while rpc.Stop is wedged) can find a live handle. handle.Kill()
// is idempotent under concurrent invocation.
func (s *subprocessConnectorShim) Stop() error {
	...
	// sync.Once-guarded close so Stop() and the host-side Kill() fallback can
	// race during the 30s timeout window without panicking on a double close.
