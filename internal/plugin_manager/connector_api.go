...
// canonicalConnectorID resolves a connector reference (canonical ID, instance
// storage key, or LegacyID) to the canonical connector ID. Returns "" if the
// reference cannot be resolved. Used by host-side policy hooks that need to
// identify a connector by its canonical ID even when a plugin invokes it via
// a legacy alias.
func (api *connectorAPI) canonicalConnectorID(ref string) string {
	if api == nil || api.pm == nil || api.pm.connectorRegistry == nil {
		return ""
	}
	def, err := api.pm.connectorRegistry.GetConnector(ref)
	if err != nil || def == nil {
		return ""
	}
	return def.ID
...
// ResolveConnectorInstanceKey maps a canonical or legacy connector ref to
// the key used in pm.connectors. Registry lookup happens outside
// connectorMu; probes run under a single connectorMu.RLock so we take one
// consistent snapshot instead of risking interleave with CreateConnectorInstance.
