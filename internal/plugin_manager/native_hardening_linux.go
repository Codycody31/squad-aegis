...
// minted per launch), so multiple plugins can share a dedicated UID without
// being able to connect to each other's RPC sockets.
...
//
// Note on PR_SET_NO_NEW_PRIVS: Go's exec.Cmd does not expose a SysProcAttr
// field for it. Run the Aegis process tree under a systemd unit with
// NoNewPrivileges=yes (or container equivalent) to inherit it into children.
...
func killProcessGroup(client *goplugin.Client) {
	...
	// ReattachConfig returns nil before start or after the process handle
	// is cleared; guard so early-failure cleanup cannot panic.
	if rc != nil && rc.Pid > 0 {
...
// IDs into a []uint32 for syscall.Credential.Groups. An empty string returns
// nil; callers should substitute an empty non-nil slice so setgroups(2) is
// still invoked, otherwise the child inherits the parent's groups.
