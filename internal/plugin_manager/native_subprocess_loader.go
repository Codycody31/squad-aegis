...
type subprocessPluginShim struct {
	...
	stopWatcherOnce *sync.Once // guards close(stopWatcher) so Stop()+Kill() races cannot double-close
...
func peekNativePluginDefinition(runtimePath, expectedSHA256 string, manifest PluginPackageManifest, target PluginPackageTarget) (PluginDefinition, error) {
	...
	// Bound the peek so a wedged plugin cannot hang installMu.
	...
	// pkg/pluginrpc.protoToConfigFieldsAtDepth already enforces this on the
	// wire decode path; this is a belt-and-suspenders check for in-process
	// constructions of ConfigField trees.
	...
	for {
		...
		case <-ticker.C:
			...
			if handle.client.Exited() {
				...
				if !intentional {
					// Only flip to Error on unintentional exits. A clean
					// Stop() already wrote PluginStatusStopped; overwriting
					// it here would mislead operators and downstream code
					// that branches on getStatus() == Error.
				...
				intentional = s.intentional
				if intentional {
					// Stop() flipped between the two reads; revert any
					// status change we just made and treat this as clean.
...
//
// The handle/svc fields are not cleared until after handle.Kill() completes,
// so a concurrent Kill() (the host-side fallback when the outer
// stopPluginInstance timeout fires while rpc.Stop is wedged) can find a live
// handle and SIGKILL the subprocess. handle.Kill() and svc.Close() are
...
func (s *subprocessPluginShim) Stop() error {
	...
	stopOnce := s.stopWatcherOnce
	watcherDone := s.watcherDone
	s.intentional = true
	s.status = PluginStatusStopped
	s.mu.Unlock()

	// Signal the watcher first so it observes the intentional flag. Guarded
	// by sync.Once because Stop() and Kill() can race during the host-side
	// 30s timeout fallback; a naive select-default-close would panic.
	if stopWatcher != nil && stopOnce != nil {
		stopOnce.Do(func() { close(stopWatcher) })
	}

	var stopErr error
	if handle != nil && handle.rpc != nil {
		// Bound the Stop RPC: if the plugin's Stop hangs, the host-side
		// stopPluginInstance timeout will call Kill() concurrently to
		// SIGKILL the subprocess and unblock this call.
...
func (s *subprocessPluginShim) ExecuteCommand(commandID string, params map[string]interface{}) (*CommandResult, error) {
	...
	// Generous cap; commands can be slow (e.g. balance teams) but must
	// never wedge the host thread forever.
...
func mergeWirePluginIntoHost(wire pluginrpc.PluginDefinition, manifest PluginPackageManifest, target PluginPackageTarget) (PluginDefinition, error) {
	...
	for _, ev := range wire.Events {
		...
		// Wildcard subscriptions ("*" or empty) bypass any per-family
		// capability gating because they are not bound to a single family.
		// Native plugins must list event types explicitly so the manifest
		// capability set is meaningful; drop wildcards rather than letting
		// them silently grant access to every event family.
		if string(evType) == "" || string(evType) == "*" {
			log.Warn().
				Str("plugin_id", manifest.PluginID).
				Msg("Native plugin requested wildcard event subscription; dropped (native plugins must list event types explicitly)")
			continue
		}
...
// Wildcard subscriptions are filtered out before this is called.
