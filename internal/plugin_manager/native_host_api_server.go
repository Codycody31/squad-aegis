...
// MaxPluginDataKeyLen bounds plugin-data keys across both the HTTP control
// surface and the gRPC HostAPI surface so a key written via one is not
...
// Close stops the HostAPI listener. Safe under concurrent invocation: the
// underlying broker stop runs at most once even when Stop() and Kill() race
// during the plugin lifecycle timeout fallback.
...
// numeric Squad player ID. AdminRemovePlayerFromSquadById expects the
// numeric form, and the player tracker may populate Player.ID with any of
// the three. This wider validator is used only by the squad-removal RPC;
// persistent flows (ban/kick/warn) keep the strict validatePlayerID.
...
func (d *hostAPIDispatcher) RconRemovePlayerFromSquadById(_ context.Context, req *pluginrpcpb.RconRemoveSquadRequest) (*pluginrpcpb.Empty, error) {
	...
	// AdminRemovePlayerFromSquadById accepts the numeric Squad ID in
	// addition to Steam64/EOS, so use the wider validator here.
...
func (d *hostAPIDispatcher) EventPublishEvent(_ context.Context, req *pluginrpcpb.PublishEventRequest) (*pluginrpcpb.Empty, error) {
	...
	// Empty event_type would namespace to "PLUGIN_<id>_" with a trailing
	// underscore that no subscriber matches.
	if strings.TrimSpace(req.GetEventType()) == "" {
	...
	// Always prepend "PLUGIN_<id>_" so plugins cannot impersonate system
	// events or each other. A HasPrefix shortcut would be ambiguous because
	// plugin IDs can contain underscores.
...
func (d *hostAPIDispatcher) ConnectorCall(rpcCtx context.Context, req *pluginrpcpb.ConnectorCallRequest) (*pluginrpcpb.ConnectorCallResponse, error) {
	...
	// Apply per-instance security checks that the connector implementation
	// itself does not see. Discord enforces a per-plugin-instance channel
	// allowlist via DiscordSendMessage/DiscordSendEmbed, but the generic
	// ConnectorAPI.Call path bypasses those wrappers and would otherwise let a
	// plugin post to any channel by routing through the connector directly.
	if err := d.enforceConnectorCallPolicy(req.GetConnectorId(), data); err != nil {
		return nil, err
	}
...
// discordConnectorID is the canonical identifier for the in-tree Discord
// connector. ConnectorCall traffic targeting this connector is subject to the
// per-instance _aegis_discord_channels allowlist, mirroring the protection
// already applied to DiscordSendMessage / DiscordSendEmbed.
const discordConnectorID = "com.squad-aegis.connectors.discord"

// enforceConnectorCallPolicy applies host-side security gates that the
// generic ConnectorAPI.Call surface would otherwise bypass. The connectorID
// is canonicalized so a plugin invoking via a LegacyID cannot dodge the
// policy. Currently scoped to the Discord channel allowlist.
func (d *hostAPIDispatcher) enforceConnectorCallPolicy(connectorID string, data map[string]interface{}) error {
	canonical := connectorID
	if resolver, ok := d.apis.ConnectorAPI.(interface {
		canonicalConnectorID(string) string
	}); ok {
		if resolved := resolver.canonicalConnectorID(connectorID); resolved != "" {
			canonical = resolved
		}
	}
	if canonical != discordConnectorID {
		return nil
	}
	...
	return d.checkDiscordChannel(channelID)
}
