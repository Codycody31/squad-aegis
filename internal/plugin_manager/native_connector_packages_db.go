...
func (pm *PluginManager) loadInstalledConnectorPackages() error {
		...
		if mustQuarantine {
			if pkg.InstallState == PluginInstallStateReady || pkg.InstallState == PluginInstallStatePendingRestart {
				if previouslyTrusted {
					...
					// Quarantine without removing the runtime file; re-upload
					// recovers. Mirrors the plugin loader path.
	...
	for _, pkg := range loaded {
		...
		// Reconcile any orphan .prev stash from an interrupted same-version
		// reinstall. Mirrors the plugin loader's logic: restore .prev if
		// runtimePath is missing, otherwise remove the stale stash so .prev
		// files don't accumulate across crashes.
		if pkg.RuntimePath != "" {
			prevPath := pkg.RuntimePath + ".prev"
			if _, statErr := os.Stat(prevPath); statErr == nil {
				safePrev, prevPathErr := validateRuntimePathWithinRoot(prevPath, connectorRuntimeDir())
				if prevPathErr != nil {
					log.Warn().Err(prevPathErr).Str("connector_id", pkg.ConnectorID).Str("path", prevPath).Msg("Refusing to reconcile native connector .prev stash outside runtime root")
					...
					if safeRuntime, runtimePathErr := validateRuntimePathWithinRoot(pkg.RuntimePath, connectorRuntimeDir()); runtimePathErr == nil {
						if renameErr := os.Rename(safePrev, safeRuntime); renameErr == nil {
							log.Info().Str("connector_id", pkg.ConnectorID).Str("path", safeRuntime).Msg("Restored native connector runtime file from .prev stash after interrupted install")
						} else {
							log.Warn().Err(renameErr).Str("connector_id", pkg.ConnectorID).Str("from", safePrev).Str("to", safeRuntime).Msg("Failed to restore native connector runtime file from .prev stash")
						}
					}
				} else {
					if removeErr := os.Remove(safePrev); removeErr != nil {
						log.Warn().Err(removeErr).Str("connector_id", pkg.ConnectorID).Str("path", safePrev).Msg("Failed to remove stale native connector .prev stash")
					} else {
						log.Info().Str("connector_id", pkg.ConnectorID).Str("path", safePrev).Msg("Removed stale native connector .prev stash from previous successful install")
					}
				}
			}
		}
