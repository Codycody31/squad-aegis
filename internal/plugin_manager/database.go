...
func (pm *PluginManager) ensurePluginInstanceContext(instance *PluginInstance) {
	// A cancelled context (left over from Disable->Stop) needs replacement;
	// otherwise event-subscription goroutines selecting on Done() would
	// exit immediately on re-enable.
...
func (pm *PluginManager) UpdateConnectorConfig(connectorID string, config map[string]interface{}) error {
	...
	// lifecycleMu blocks a concurrent Delete from racing UpdateConfig+Start
	// and leaving the connector running after the row is gone.
	instance.lifecycleMu.Lock()
	defer instance.lifecycleMu.Unlock()
