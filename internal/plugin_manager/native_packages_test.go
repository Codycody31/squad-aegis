...
func TestVerifyManifestSignatureRejectsUntrustedKey(t *testing.T) {
	...
	// Untrusted key yields verified=false with no error so the
	// AllowUnsafeSideload gate applies uniformly to "unsigned" and
	// "signed by wrong key".
...
// TestVerifyManifestSignatureRejectsRevokedPublicKey guards the CRL
// fingerprint path: revoking a compromised public key by SHA256 fingerprint
// must reject signatures from that key even when the bundle declares a fresh
// key_id label that is not on the operator's revoked-key_id list. Without
// this, an attacker who steals a trusted signing key can simply pick a new
// label per bundle and bypass the CRL.
func TestVerifyManifestSignatureRejectsRevokedPublicKey(t *testing.T) {
	publicKey, privateKey, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("ed25519.GenerateKey() error = %v", err)
	}
	...
	publicKeyRaw, err := plugin_signing.EncodePublicKeyFile(publicKey)
	if err != nil {
		t.Fatalf("EncodePublicKeyFile() error = %v", err)
	}

	fingerprint := publicKeyFingerprint(publicKeyRaw)
	if fingerprint == "" {
		t.Fatal("publicKeyFingerprint() returned empty for valid key")
	}
	resetRevokedFingerprintsForTest(fingerprint)
	t.Cleanup(func() {
		resetRevokedKeyIDsForTest()
		resetRevokedFingerprintsForTest()
	})

	manifestRaw := []byte(`{"plugin_id":"com.example.fp-revoked","version":"1.0.0"}`)
	signedAt := time.Now().UTC()
	// Note: a label NOT on the revoked-key_id list, so only the fingerprint
	// CRL can stop this bundle.
	signedPayload, err := plugin_signing.BuildSignedPayload(manifestRaw, "fresh-label-attacker-controls", signedAt, signedAt.Add(time.Hour))
	if err != nil {
		t.Fatalf("BuildSignedPayload() error = %v", err)
	}
	signatureRaw, _, err := plugin_signing.SignSignedPayload(signedPayload, privateKey)
	if err != nil {
		t.Fatalf("SignSignedPayload() error = %v", err)
	}
	...
	if verification.Verified {
		t.Fatal("verifyManifestSignature() verified = true, want false for revoked public key fingerprint")
	}
}

// TestVerifyManifestSignaturePreservesExplicitDefaults guards against the
// signature-verification regression where the host re-marshaled the parsed
// PluginPackageManifest (Official is `omitempty`) and then handed those bytes
// to the verifier. A signed bundle whose manifest.json contains an explicit
// `"official": false` is canonically distinct from `{}` after the round-trip,
// so the embedded canonical manifest no longer matched and every otherwise-
// valid bundle was rejected as "signed payload manifest does not match
// bundle manifest.json". This test pins the verifier to the raw bundle
// bytes (parts.ManifestBytes) so explicit default fields survive.
func TestVerifyManifestSignaturePreservesExplicitDefaults(t *testing.T) {
	publicKey, privateKey, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("ed25519.GenerateKey() error = %v", err)
	}
	...
	// manifest.json emitted by the publisher's signing tool: it includes
	// "official": false explicitly. The Go struct has `omitempty`, so a
	// re-marshal would drop the field and break the canonical compare.
	manifestRaw := []byte(`{"plugin_id":"com.example.with-defaults","name":"With Defaults","version":"1.0.0","official":false,"targets":[{"min_host_api_version":1,"target_os":"linux","target_arch":"amd64","library_path":"plugin.so","sha256":"deadbeef"}]}`)
	...
	verification, err := verifyManifestSignature(signedPayload, manifestRaw, signatureRaw, publicKeyRaw)
	if err != nil {
		t.Fatalf("verifyManifestSignature() with raw manifest bytes returned error = %v", err)
	}
	if !verification.Verified {
		t.Fatal("verifyManifestSignature() = false on raw manifest bytes; signed bundle with explicit `\"official\": false` would be rejected at install time")
	}

	// And confirm the regression mode: a struct round-trip that drops
	// explicit defaults must NOT verify. This pins the install pipeline to
	// pass raw bundle bytes, never re-marshaled struct bytes. Canonical
	// mismatch is surfaced as an error per verifyManifestSignature's
	// contract, which is what we expect here.
	var parsed PluginPackageManifest
	if err := json.Unmarshal(manifestRaw, &parsed); err != nil {
	...
	roundTripVerification, roundTripErr := verifyManifestSignature(signedPayload, roundTripped, signatureRaw, publicKeyRaw)
	if roundTripErr == nil && roundTripVerification.Verified {
		t.Fatal("verifyManifestSignature() = true on re-marshaled manifest; the install pipeline must verify against raw bundle bytes")
	}
}
