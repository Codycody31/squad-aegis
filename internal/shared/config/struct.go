...
type Struct struct {
	...
	Plugins struct {
		...
		// {
		//   "revoked_key_ids": ["ops-key-2025-q4", ...],
		//   "revoked_public_key_fingerprints": ["<sha256-hex of public key bytes>", ...]
		// }
		// Empty disables the CRL. The file is re-read every
		// RevokedKeyIDsRefreshSeconds. Fingerprint revocation is the
		// authoritative way to retire a compromised key; key_id revocation
		// alone is bypassable by re-signing with a fresh label.
