...
func (e *DiscordTeamkillExtension) handleTeamkill(data interface{}) error {
	teamkillEvent, ok := data.(logwatcher.TeamkillEvent)
	if !ok {
		return fmt.Errorf("data is not a TeamkillEvent")
	}
	...
	for _, player := range players.OnlinePlayers {
		if player.SteamId == teamkillEvent.AttackerSteam {
			attacker = &player
		}
		if strings.Contains(player.Name, teamkillEvent.VictimName) {
			victim = &player
		}
	}
	...
	embed := &discordgo.MessageEmbed{
		Title: fmt.Sprintf("Teamkill: %s", attacker.Name),
		Color: color,
		Fields: []*discordgo.MessageEmbedField{
			...
			{
				Name:   "Weapon",
				Value:  teamkillEvent.Weapon,
				Inline: false,
			},
			{
				Name:   "Victim's Name",
				Value:  teamkillEvent.VictimName,
				Inline: true,
			},
			{
