...
const fetchAllServerStats = async () => {
  ...
  const promises = servers.value.map(async (server) => {
    ...
    const { data: metricsData } = await useFetch(
      `${runtimeConfig.public.backendApi}/servers/${server.id}`,
      {
        headers: {
          Authorization: `Bearer ${
            useCookie(runtimeConfig.public.sessionCookieName).value
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <h3 class="text-3xl font-bold mt-1">{{ stats.totalServers }}</h3>
            </div>
            <div
          ...
          <div class="mt-4">
            <p class="text-sm text-muted-foreground">
              <span class="text-green-500">{{ stats.activeServers }}</span>
              servers online
            </p>
          </div>
        </CardContent>
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <h3 class="text-3xl font-bold mt-1">{{ stats.totalPlayers }}</h3>
            </div>
            <div
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <h3 class="text-3xl font-bold mt-1">{{ stats.totalBans }}</h3>
            </div>
            <div
      ...
      <Card>
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <h3 class="text-3xl font-bold mt-1 text-green-500">
                {{
                  Object.values(serverStatus).every((status) => status.rcon)
                    ? "Healthy"
                    : "Degraded"
                }}
              </h3>
          ...
          <div class="mt-4">
            <p class="text-sm text-muted-foreground">
              {{
                Object.values(serverStatus).some((status) => !status.rcon)
            ...
            <TableRow v-for="server in servers" :key="server.id">
              ...
              <TableCell>
                <span
                  ...
                  "
                >
                  {{ serverStatus[server.id]?.rcon ? "Online" : "Offline" }}
                </span>
              </TableCell>
