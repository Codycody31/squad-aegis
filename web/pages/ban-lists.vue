...
const loadRemoteSources = async () => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.backendApi}/remote-ban-sources`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    }) as any;
...
const loadIgnoredSteamIDs = async () => {
  ...
  try {
    const response = await $fetch(`${runtimeConfig.public.backendApi}/ignored-steam-ids`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    }) as any;
...
const updateBanList = async () => {
  ...
  try {
    await $fetch(`${runtimeConfig.public.backendApi}/ban-lists/${currentBanList.value.id}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
const deleteBanList = async (banList: any) => {
  if (!confirm(`Are you sure you want to delete the ban list "${banList.name}"?`)) {
    return;
  }
  ...
  try {
    await $fetch(`${runtimeConfig.public.backendApi}/ban-lists/${banList.id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
const deleteRemoteSource = async (remoteSource: any) => {
  if (!confirm(`Are you sure you want to delete the remote source "${remoteSource.name}"?`)) {
    return;
  }
  ...
  try {
    await $fetch(`${runtimeConfig.public.backendApi}/remote-ban-sources/${remoteSource.id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
const deleteIgnoredSteamID = async (ignoredSteamID: any) => {
  if (!confirm(`Are you sure you want to remove Steam ID "${ignoredSteamID.steam_id}" from the ignore list?`)) {
    return;
  }
  ...
  try {
    await $fetch(`${runtimeConfig.public.backendApi}/ignored-steam-ids/${ignoredSteamID.id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600 mt-2">
          Manage ban lists, remote sources, and ignored Steam IDs for shared ban coordination.
        </p>
      </div>
    ...
    <Card>
      <CardHeader>
        <div class="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Manage local and remote ban lists that can be shared across servers.
            </CardDescription>
          </div>
          <Dialog v-model:open="showCreateDialog">
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new ban list that can be shared with other servers.
                </DialogDescription>
              </DialogHeader>
              <div class="grid gap-4 py-4">
                ...
                <div class="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor="is_remote">Remote Ban List</Label>
                </div>
                ...
                <div v-if="banListForm.is_remote" class="flex items-center space-x-2">
                  <Switch
                    id="remote_sync_enabled"
                    v-model="banListForm.remote_sync_enabled"
                  />
                  <Label htmlFor="remote_sync_enabled">Enable Auto Sync</Label>
                </div>
              </div>
              <DialogFooter>
      ...
      <CardContent>
        ...
        <div v-else-if="banLists.length === 0" class="text-center py-8">
          <p class="text-gray-500">No ban lists found. Create your first ban list to get started.</p>
        </div>
        <div v-else>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow v-for="banList in banLists" :key="banList.id">
                <TableCell class="font-medium">{{ banList.name }}</TableCell>
                <TableCell>{{ banList.description || "No description" }}</TableCell>
                <TableCell>
                  <Badge :variant="banList.is_remote ? 'secondary' : 'default'">
                    {{ banList.is_remote ? "Remote" : "Local" }}
                  </Badge>
                </TableCell>
                <TableCell>{{ new Date(banList.created_at).toLocaleDateString() }}</TableCell>
                <TableCell class="text-right">
                  <div class="flex gap-2 justify-end">
                    <Button variant="outline" size="sm" @click="editBanList(banList)">
                      <Edit class="h-4 w-4" />
                    </Button>
                    <Button variant="destructive" size="sm" @click="deleteBanList(banList)">
    ...
    <Card>
      <CardHeader>
        <div class="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Configure external ban list sources to automatically sync bans from other communities.
            </CardDescription>
          </div>
          <Dialog v-model:open="showRemoteSourceDialog">
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a remote ban source to automatically sync bans from other communities.
                </DialogDescription>
              </DialogHeader>
              <div class="grid gap-4 py-4">
                <div>
                ...
                <div>
                  <Label htmlFor="remote_source_url">URL</Label>
                  <Input
                ...
                <div>
                  ...
                  <Input
                    id="sync_interval"
                    type="number"
                    v-model="remoteSourceForm.sync_interval_minutes"
                    min="60"
                    max="1440"
                    placeholder="180"
                  />
              ...
              <DialogFooter>
                <Button variant="outline" @click="showRemoteSourceDialog = false">
                  Cancel
                </Button>
      ...
      <CardContent>
        <div v-if="loading && remoteSources.length === 0" class="text-center py-8">
          <div class="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-4"></div>
          <p>Loading remote sources...</p>
        </div>
        <div v-else-if="remoteSources.length === 0" class="text-center py-8">
          <p class="text-gray-500">No remote sources configured. Add remote sources to sync bans from other communities.</p>
        </div>
        <div v-else>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow v-for="source in remoteSources" :key="source.id">
                <TableCell class="font-medium">{{ source.name }}</TableCell>
                <TableCell class="font-mono text-sm">{{ source.url }}</TableCell>
                <TableCell>
                  <Badge :variant="source.sync_enabled ? 'default' : 'secondary'">
                    {{ source.sync_enabled ? "Enabled" : "Disabled" }}
                  </Badge>
                </TableCell>
                <TableCell>{{ source.sync_interval_minutes }} min</TableCell>
                <TableCell>
                  {{ source.last_synced_at ? new Date(source.last_synced_at).toLocaleString() : "Never" }}
                </TableCell>
                <TableCell class="text-right">
                  <Button variant="destructive" size="sm" @click="deleteRemoteSource(source)">
                    <Trash2 class="h-4 w-4" />
                  </Button>
                </TableCell>
              </TableRow>
    ...
    <Card>
      <CardHeader>
        <div class="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Manage Steam IDs that should be ignored when syncing from remote ban sources.
            </CardDescription>
          </div>
          <Dialog v-model:open="showIgnoredSteamIDDialog">
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a Steam ID to ignore when syncing bans from remote sources.
                </DialogDescription>
              </DialogHeader>
              <div class="grid gap-4 py-4">
                <div>
              ...
              <DialogFooter>
                <Button variant="outline" @click="showIgnoredSteamIDDialog = false">
                  Cancel
                </Button>
      ...
      <CardContent>
        <div v-if="loading && ignoredSteamIDs.length === 0" class="text-center py-8">
          <div class="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-4"></div>
          <p>Loading ignored Steam IDs...</p>
        </div>
        <div v-else-if="ignoredSteamIDs.length === 0" class="text-center py-8">
          <p class="text-gray-500">No Steam IDs are currently ignored. Add Steam IDs to prevent them from being banned via remote sources.</p>
        </div>
        <div v-else>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow v-for="ignoredID in ignoredSteamIDs" :key="ignoredID.id">
                <TableCell class="font-mono">{{ ignoredID.steam_id }}</TableCell>
                <TableCell>{{ ignoredID.reason || "No reason provided" }}</TableCell>
                <TableCell>{{ new Date(ignoredID.created_at).toLocaleDateString() }}</TableCell>
                <TableCell class="text-right">
                  <Button variant="destructive" size="sm" @click="deleteIgnoredSteamID(ignoredID)">
                    <Trash2 class="h-4 w-4" />
                  </Button>
                </TableCell>
              </TableRow>
        ...
        <div class="grid gap-4 py-4">
          ...
          <div v-if="banListForm.is_remote" class="flex items-center space-x-2">
            <Switch
              ...
              v-model="banListForm.remote_sync_enabled"
            />
            <Label htmlFor="edit_remote_sync_enabled">Enable Auto Sync</Label>
          </div>
        </div>
        <DialogFooter>
