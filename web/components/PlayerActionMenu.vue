...
async function fetchEscalationSuggestion(steamId: string, ruleId: string) {
  ...
  try {
    ...
    if (fetchError.value) {
      console.error("Failed to fetch escalation suggestion:", fetchError.value);
      escalationSuggestion.value = null;
    } else if (
      data.value &&
      ...
      if (
        ...
        const rule = serverRules.value.find((r) => r.id === suggestion.rule_id);
        if (rule) {
...
function switchToSuggestedAction() {
  ...
  if (
    ...
    if (
      ...
      actionDuration.value = escalationSuggestion.value.suggested_duration;
    ...
    if (!actionReason.value) {
      if (escalationSuggestion.value.suggested_message) {
        actionReason.value = escalationSuggestion.value.suggested_message;
    ...
    <DropdownMenuContent align="end">
      ...
      <template v-if="player.sinceDisconnect == ''">
        <DropdownMenuItem
          ...
          v-if="authStore.getServerPermission(serverId as string, 'warn')"
        >
          <Icon
        ...
        <DropdownMenuItem
          ...
          v-if="authStore.getServerPermission(serverId as string, 'kick') && player.squadId != 0"
        >
        ...
        <DropdownMenuItem
          ...
          v-if="authStore.getServerPermission(serverId as string, 'kick')"
        >
  ...
  <Dialog v-model:open="showActionDialog">
    <DialogContent class="sm:max-w-[425px]">
      ...
      <div class="grid gap-4 py-4">
        ...
        >
          <label for="rule" class="text-right col-span-1">Rule Violation</label>
          <Select
            v-model="selectedRuleId"
            @update:model-value="(value) => onRuleSelected(value as string)"
          >
            <SelectTrigger class="col-span-3">
              <SelectValue placeholder="Optional: Select a violated rule" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__"> No rule violation </SelectItem>
              <SelectItem
        ...
        <div
          v-if="actionType !== 'move' && actionType !== 'remove-from-squad'"
          class="grid grid-cols-4 items-center gap-4"
        >
          ...
          <Textarea
            ...
            :placeholder="
              actionType === 'warn' ? 'Warning message' : 'Reason for action'
            "
            class="col-span-3"
            rows="3"
          />
        ...
        >
          Are you sure you want to remove {{ player.name }} from their squad?
        </div>
      </div>
          ...
          <div class="flex items-start space-x-2">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-blue-700 mt-1">
                ...
                time(s).
                <template v-if="escalationSuggestion.suggested_action">
                  Suggested action:
                  <strong>{{ escalationSuggestion.suggested_action }}</strong>
                  <template v-if="escalationSuggestion.suggested_duration">
                    for
                    <strong
                      >{{
