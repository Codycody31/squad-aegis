...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/dist/wasm-plugin-hostcall-demo}"

mkdir -p "${OUTPUT_DIR}/bin/wasm"

echo "Building wasm-plugin-hostcall-demo (requires: rustc + wasm32-unknown-unknown)..."
(
  cd "${ROOT_DIR}"
  rustup target add wasm32-unknown-unknown 2>/dev/null || true
  rustc --edition 2021 --crate-type cdylib "${ROOT_DIR}/examples/wasm-plugin-hostcall-demo/plugin.rs" \
...
if command -v jq >/dev/null 2>&1; then
  jq --arg sha "${SHA256}" '(.targets[0].sha256) = $sha' \
    "${ROOT_DIR}/examples/wasm-plugin-hostcall-demo/manifest.json" > "${MANIFEST_OUT}"
else
  python3 -c "import json,sys; m=json.load(open(sys.argv[1])); m['targets'][0]['sha256']=sys.argv[2]; json.dump(m, open(sys.argv[3],'w'), indent=2); open(sys.argv[3],'a').write(chr(10))" \
    "${ROOT_DIR}/examples/wasm-plugin-hostcall-demo/manifest.json" "${SHA256}" "${MANIFEST_OUT}"
fi

TARGET_LABEL="wasm-wasm"
ZIP_PATH="${OUTPUT_DIR}/wasm-hostcall-demo-${TARGET_LABEL}.zip"
rm -f "${ZIP_PATH}"
(
  cd "${OUTPUT_DIR}"
